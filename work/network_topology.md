...
' ===== 全局色板 =====
skinparam backgroundColor #0B1320
skinparam defaultFontColor #E6F1FF
...
' ===== 边缘节点（冷蓝灰） =====
rectangle "VIU_ML\n左侧区域/车身控制单元" as VIUML #11203A
rectangle "TBOX\n车联网通信单元" as TBOX #11203A
rectangle "VIU_MR\n右侧区域/车身控制单元" as VIUMR #11203A
rectangle "LRR_F\n前向毫米波雷达" as LRR #11203A
rectangle "PBox\n动力域盒" as PBOX #11203A
rectangle "FLL\n前视感知" as FLL #11203A

' ===== 核心节点（高亮渐变感） =====
rectangle "VCM\n车控管理 / 车载以太网中央交换" as VCM #1A2E52
rectangle "ADCU A1/A2\n自动驾驶控制器（主芯片 J6P）" as ADCU #1A2E52

' ===== 焦点节点（霓虹绿青） =====
rectangle "IVI（本文所述座舱系统）\nSA8397 — PVM (Linux) + GVM (Android)" as IVI #0E3B3B
...
```
              ┌──────────────┐                                    ┌──────────────┐
              │     VCM      │                                    │     ADCU     │
              │ (trunk peer) │                                    │ (video peer) │
              └──────┬───────┘                                    └──────┬───────┘
                     │ 802.1Q Trunk                                      │ Access
                     │ VLAN 3 / 4 / 6 / 7 / 8 / 10..14                   │ VLAN 15 / 19
                     ▼                                                   ▼
            (via SoC emac1 + RTL9071 board switch, see §3.2.2/§3.2.3)
            ┌───────────────────┐                            ┌───────────────────┐
            │   PVM eth1        │                            │   PVM eth0        │
            │   (Phys NIC #1)   │                            │   (Phys NIC #2)   │
            │   MAC 02:df:53:.4 │                            │   MAC 02:df:53:.9 │
            └─────────┬─────────┘                            └─────────┬─────────┘
                      │ 802.1Q sub-IF                                  │
                      └───────────────────────┬────────────────────────┘
                                              ▼
        ┌────────────────────────── PVM Physical VLAN Sub-IFs ─────────────────────────┐
        │   eth1.3       eth1.4       eth1.6       eth1.7       eth1.8                 │
        │   172.16.      172.16.      172.16.      172.16.      172.16.                │
        │   103.40       104.40       106.40       107.40       108.40                 │
        │                                                                              │
        │   eth1.10      eth1.11      eth1.12      eth1.13      eth1.14                │
        │   172.16.      172.16.      172.16.      172.16.      172.16.                │
        │   110.40       111.40       112.40       113.40       114.40                 │
        │                                                                              │
        │   eth0.15      eth0.19                                                       │
        │   172.16.      172.16.                                                       │
        │   115.41       119.41                                                        │
        ...
        │  ──────────────────────────────────────────────────────────────────────────  │
        │   • Kernel routes  : main + table 106 / 107 / 108                            │
        │   • iptables nat   : DNAT + bidirectional SNAT                               │
        │   • iptables filter: FORWARD (stateful)                                      │
        │   • Policy routing : iif vmtap1.X  ->  table 10X                             │
        └───────────────────────────────────────┬──────────────────────────────────────┘
                                                ▼
        ┌──────────────── PVM vmtap (Host-side virtio backend) ────────────────────────┐
        │   vmtap0      vmtap1.3     vmtap1.4     vmtap1.6     vmtap1.7    vmtap1.8    │
        │   10.10.      10.10.       10.10.       10.10.       10.10.      10.10.      │
        │   200.1       103.1        104.1        106.1        107.1       108.1       │
        └───────────────────────────────────────┬──────────────────────────────────────┘
                                                │
                                    ════════ virtio-net ════════
                                                │
                                                ▼
        ┌──────────────────── GVM (Android) virtio frontend ───────────────────────────┐
        │   eth0        eth1.3       eth1.4       eth1.6       eth1.7      eth1.8      │
        │   10.10.      10.10.       10.10.       10.10.       10.10.      10.10.      │
        │   200.40      103.40       104.40       106.40       107.40      108.40      │
        │  ──────────────────────────────────────────────────────────────────────────  │
        │   Routing: policy routing (per-network table) + Android multinetwork         │
        └──────────────────────────────────────────────────────────────────────────────┘
```
...
```
   ...
   └─────────────────────────┼───────────────────────┼───────────┼─────┘
                             │                       │           │
                  ════ SoC boundary (package pins) ════════════════════
                             │                       │           │
                  PCB trace ─┼───────────────────────┼───────────┼─────►
                             ▼                       ▼           ▼
                       RTL9071CP-VB-CG board switch IC (see §3.2.3)
                             P11                    P9          P10
                      (ADCU video link)        (VCM trunk)   (1G spare)
```
...
- **HSGMII**：SGMII 的 2.5G 扩展（Cisco 命名），也可降速运行 1G/100M。

##### 物理连接方式与板级走线

SoC 与 RTL9071 之间**完全是 PCB 板内走线**，不经过任何连接器或线缆：

| 段 | 物理介质 | 信号 | 说明 |
| --- | --- | --- | --- |
| SoC ↔ RTL9071 | **PCB 上的高速差分对**（板内走线，长度 ≤ 数十 mm） | USXGMII / PCIE / HSGMII（SerDes 差分对，每对 TX±/RX±） | 板内 BGA 引脚到 BGA 引脚直连，需做阻抗匹配（通常 100 Ω 差分）、长度补偿、参考层完整；无连接器 |
| RTL9071 ↔ 车外 ECU（VCM / ADCU / …） | **车载以太网单对双绞线**（UTP，1 pair / 2 wires） | 100/1000BASE-T1（PAM3 编码，单对全双工） | 板上 BASE-T1 PHY 引脚 → 板载车规连接器（H-MTD / MATEnet 等）→ 车内线束 → 对端 ECU 板上的 BASE-T1 PHY；最长约 15 m |

要点：

- **SoC 出片不直接驱动车载双绞线**：SoC 的 USXGMII/PCIE/HSGMII 都是 SerDes 信号，只在 PCB 内传输；必须经过 RTL9071 的 BASE-T1 PHY 把信号转成 100/1000BASE-T1，才能上车载线束。
- **车载以太网 ≠ 普通网线**：用单对非屏蔽双绞线 + 车规连接器（H-MTD / MATEnet 等），不是家用 4 对 RJ45 网线；电气与编码也不一样（PAM3 vs PAM5/NRZ），所以不能与办公网设备直连。
- **两条出板链路物理隔离**：
  - `emac1 → USXGMII → RTL9071 P9 → P5 → 1000BASE-T1 双绞线 → VCM`（车控 trunk）
  - `emac0 → PCIE  → RTL9071 P11 → P6 → 1000BASE-T1 双绞线 → ADCU`（视频专线）

  两条链路在板上、连接器、线束上都各走各的，不共享对绞线，避免高带宽 RTSP 视频流挤占车控总线。
- **预留链路**：`SAIL → HSGMII → RTL9071 P10` 当前在 PVM 中未启用，对应板载 BASE-T1 端口可能未引出连接器或未使用。
...
```
            ...
            │            TAP Device             │
            │  ┌─────────────────────────────┐  │
            │  │ Kernel side: Ethernet iface │  │
            │  │ (assignable IP/MAC,         │  │
            │  │  visible in routing table)  │  │
            │  └──────────────┬──────────────┘  │
            └─────────────────┼─────────────────┘
                              │ /dev/net/tun (char device)
            ┌─────────────────┼─────────────────┐
            │   Userspace process (VMM, e.g.    │
            │   crosvm)                         │
            │   read()  ← receive Ethernet      │
            │              frames from kernel   │
            │   write() → inject Ethernet       │
            │              frames into kernel   │
            └───────────────────────────────────┘
```
...
```
                   ┌──── External (in-vehicle net) ────┐
                   │ src = BizECU                      │
                   │ dst = 172.16.106.40               │
                   └────────────────┬──────────────────┘
                                    │ ingress on PVM eth1.6
                                    ▼
        PREROUTING: -d 172.16.106.40 -i eth1.6
                    -j DNAT --to 10.10.106.40
                                    │
                                    ▼  dst rewritten to 10.10.106.40
              [Route decision] → matches main table:
                                  10.10.106.0/24 dev vmtap1.6
                                    │
                                    ▼  before egress on vmtap1.6
       POSTROUTING: -d 10.10.106.40 -o vmtap1.6
                    -j SNAT --to 10.10.106.1
                                    │
                                    ▼  src rewritten to 10.10.106.1
                   ┌──── Ingress on GVM eth1.6 ────────┐
                   │ src = 10.10.106.1 (gateway)       │
                   │ dst = 10.10.106.40                │
                   └───────────────────────────────────┘

           Android sees: "my gateway 10.10.106.1 forwarded a
                          connection to me"
                                    ▲
                                    │ Reply path reversed; conntrack
