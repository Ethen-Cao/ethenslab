...
rectangle "ZCU_L\n左区域控制器" as ZCUL #ECEFF1
rectangle "ZCU_R\n右区域控制器" as ZCUR #ECEFF1
...
| **OBD** | On-Board Diagnostics | 标准车载诊断接口（外部诊断仪、检测设备物理接入点） |
| **ZCU_L / ZCU_R** | Zone Control Unit (Left/Right) | 左右区域控制器，下挂车身、舒适、灯光等域内 ECU |
| **ADCU J6P** | Automated Driving Control Unit | 自动驾驶域控，主芯片为地平线 J6P（旁挂感知件 + 直连 IVI 视频专线） |
| **ADCU 中低配预留** | — | 中低配版本可选的次级 ADCU，物理位置预留、当前未启用 |
| **LRR_F** | Long Range Radar Front | 前向毫米波雷达（接入 ADCU；当前预留状态） |
...
2. **`ETH 1000BASE-RTSP FWC`**——视频专线，由 ADCU J6P 直连 IVI（图中虚线，绕过 VCM），承载摄像头/感知视频流（RTSP，FWC = Forward/Front-View Camera 等推测）。
...
| eth1 | 6 | `eth1.6` | `172.16.106.40/24` | 业务通道（含 10.82.13.0/24、10.92.89.8 路由） |
| eth1 | 7 | `eth1.7` | `172.16.107.40/24` | Android 透传业务面 |
| eth1 | 8 | `eth1.8` | `172.16.108.40/24` | Android 透传业务面 |
...
| eth1 | 14 | `eth1.14` | `172.16.114.40/24` | SOME/IP 服务总线 |
| eth0 | 15 | `eth0.15` | `172.16.115.41/24` | PVM 控制/管理 |
| eth0 | 19 | `eth0.19` | `172.16.119.41/24` | SOME/IP 部分服务 |

> **注意**：上述 5 条 PVM-only VLAN（10/11/12/13/14、15/19）均**未在 iptables 中配置 DNAT/SNAT**，对 GVM 完全不可见。这是设计上的"PVM 私有面"。
...
- `default via 172.16.103.20 dev eth1.3` 是**整个 PVM 自身**未匹配策略路由时的兜底出口。`172.16.103.20` 推测为车联网关（T-Box）地址。
...
| --- | --- | --- | --- |
| 3 | 172.16.103.40 | 10.10.103.40 | **实测**：PVM 默认网关 (`172.16.103.20`)；Android 未标记流量也下沉至此（rule 31000）。**推测**：T-Box / 互联网网关 |
| 4 | 172.16.104.40 | 10.10.104.40 | **实测**：GVM 侧 DoIP (13400)、VLM (5062/5064)、gftpd (58046)；PVM 侧 IDPS (30006) 旁路监听。**结论**：诊断与边界防御网段 |
| 6 | 172.16.106.40 | 10.10.106.40 | **实测**：Android 路由表中有 `10.82.13.0/24`、`10.92.89.8` 远端路由。**推测**：业务专网 / 云直连通道 |
| 7 | 172.16.107.40 | 10.10.107.40 | **实测**：GVM rkstack DHCPv6 客户端在线，PVM 有策略表 107。**当前**：未发现用户态 socket 绑定。**结论**：链路活跃但应用层暂未启用，非"预留" |
| 8 | 172.16.108.40 | 10.10.108.40 | 同 VLAN 7 |
| 10–14 | 172.16.110–114.40 | — | **实测**：PVM `someipd` 多实例监听（组播 239.5.1.2 + 单播 51000/51001/30490/30505/57001）。**结论**：SOME/IP 服务总线（ECU 之间），不向 GVM 暴露 |
| 15 | 172.16.115.41 | — | **推测**：PVM 控制/管理面，未发现用户态 socket |
| 19 | 172.16.119.41 | — | **实测**：`someipd` 在该 IP 监听 30490/55005。**结论**：SOME/IP 子集，不向 GVM 暴露 |
...
### 8.1 PVM 私有面（GVM 完全无法访问）

- 物理 VLAN 10–14 / 15 / 19：未在 iptables 配置 NAT，PVM 不为这些网段建立到 GVM 的转发路径。GVM 的路由表与 `ip rule` 中也无对应条目。
- PVM 监听在 `172.16.110–114.40`、`172.16.119.41` 上的 `someipd` 等服务，对 GVM 不可达。
...
- `vmtap0` ↔ Android `eth0` (`10.10.200.0/24`)：纯 PVM/GVM 内部管道，**无 NAT 出口**，外部无法到达。
...
node "PVM (Linux Host)\nip_forward=1, iptables NAT/FORWARD" as PVM #E8F5E9 {
  ...
  interface "eth0.15 / eth0.19\n172.16.115/119.41" as P_e0x
...
V78 --> P_e17 : VLAN 7
V78 --> P_e18 : VLAN 8
SOA <--> P_e1x : VLAN 10-14（GVM 不可见）
SOA <--> P_e0x : VLAN 15/19
...
1. **`172.16.103.20` 是否为 T-Box / 互联网网关**：尚未通过 ARP/抓包确认。建议通过 PVM `arp -an | grep 172.16.103.20` 与 `tcpdump -ni eth1.3` 进一步确认。
2. **VLAN 7 / VLAN 8 业务归属**：链路已就绪（DHCPv6 客户端在线、策略路由完整），但当前没有用户态进程绑定到这些 IP。需结合产品域设计明确"为何预先打通"。
3. **VLAN 6 中的 `10.82.13.0/24` 与 `10.92.89.8`**：明显是远端业务子网（非车内 172.16.X 段）。需要从车控网络规划层面确认其语义（可能是云专线 / 跨域路由）。
...
|   6    | 106.40          | 106.40           | 业务/云专线        |
|   7    | 107.40          | 107.40           | 透传，业务待启      |
|   8    | 108.40          | 108.40           | 透传，业务待启      |
|  10-14 | 110-114.40      | —                | SOME/IP 总线       |
|  15    | 115.41          | —                | PVM 控制面         |
|  19    | 119.41          | —                | SOME/IP 子集       |
...
**TAP 设备**是 Linux 内核提供的**虚拟二层网卡**（对照 TUN 设备是虚拟三层接口）。其工作方式：

- 在内核中表现为一张普通以太网接口（可分配 MAC、加入 VLAN、设置 IP、被路由表引用）。
- 在用户态打开 `/dev/net/tun` 后，所有发往该接口的以太网帧会被读到用户态文件描述符；用户态写入的字节会被作为以太网帧"注入"内核协议栈。

**典型用途**：VMM（QEMU、crosvm 等）把 Guest 网卡的后端映射到 Host 上的 TAP，实现 Guest ↔ Host 的二层连通。

**vmtap**：是 Qualcomm `qcrosvm` 在 PVM 上为每个 Guest 虚拟网卡创建的 TAP 设备**惯用命名**（`vm` 前缀代表 virtual machine）。除了名字之外，它就是一张普通 TAP，能像物理网卡一样：

- 被 `ip addr` 分配 IP（本系统中 `vmtap0=10.10.200.1`、`vmtap1.3=10.10.103.1` 等）。
- 被 `ip link` 加上 802.1Q 子接口（本系统中的 `vmtap1.3 ... vmtap1.8`）。
- 出现在路由表里、被 iptables 规则按 `-i vmtap1.X` / `-o vmtap1.X` 匹配。

**本系统的关键设计选择**：

- 没有把 vmtap 桥接（bridge）到物理网卡，而是给 vmtap **分配独立 IP（10.10.X.1）**，让 PVM 内核作为三层路由器在 vmtap 与 eth1.X 之间做 IP 转发。
- 因此 PVM↔GVM 的数据是**三层 IP 包**（在 PVM 看来），而非二层桥接帧。这是把 §4.5 NAT 设计成立的前提。
